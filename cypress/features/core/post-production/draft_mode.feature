Feature: Draft Mode
    ...
    #-------------------------------------------------------------------------------------
    # 20.1 The system shall allow production draft mode changes (Control Center) to be approved automatically under certain conditions.  Options are specified as follows: 
    #     20.1.1 Never (always require an admin to approve changes) 
    #     20.1.2 Yes, if no existing fields were modified 
    #     20.1.3 Yes, if project has no records OR if has records and no existing fields were modified 
    #     20.1.4 Yes, if no critical issues exist 
    #     20.1.5 Yes, if project has no records OR if has records and no critical issues exist 
    # 20.2 The system shall allow the option to add or modify events and arms on the Define My Events page for longitudinal projects while in production status by providing two options: 
    #     20.2.1 No, only Administrators can add/modify events in production 
    #     20.2.2 Yes, normal users can add/modify events in production 
    # 20.3 The system shall require changes made to data collection instruments in production status projects to be made only by entering draft mode.  Changes in draft mode are not implemented to the project instruments in real time. 
    # 20.4 The system shall provide detailed summary of all drafted changes. 
    # 20.5 The system shall require administrators to review changes made in draft mode if any critical issues exist AND the project contains one or more records.  
    # 20.6 The system shall provide the option to require administrators to review any changes made in draft mode 
    # 20.7 The system shall allow administrators to commit changes, reject changes, reset and delete the drafted changes.                                            
    # 20.8 The system shall flag any changes that may negatively impact data with the following critical warnings: 
    #     20.8.1 Possible label mismatch 
    #     20.8.2 Possible data loss 
    #     20.8.3 Data WILL be lost 
    # 20.9 The system shall record all versions of the data dictionary post-production with date time stamp, requestor and approver. 
    # 20.10 The system shall allow for a confirmation email to be sent to the requestor which is templated but allows for additional information to be entered. 
    # 20.11 The system shall provide the option to require administrators to review the addition or modification of events for longitudinal projects while in production  
    #-------------------------------------------------------------------------------------
