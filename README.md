...
2. **`cypress-tests`** (matrix of `SHARD_TOTAL=8` shards) — each shard `needs:` the build and pulls that **exact per-commit SHA** tag (never `:latest`, avoiding the old build/test race). It pulls the `redcap-aio` image (`:latest`), boots it (`-p 8443:8443 -p 8025:8025`, volume `cctc_mariadb_data`), waits for HTTPS on `:8443`, then runs the runner container over `--network host` with the Docker socket mounted. Both pulls are **staggered by shard index and retried with backoff**: 8 shards authenticating as the same account and pulling the same multi-GB image simultaneously trips GHCR's secondary rate limit, which surfaces mid-transfer as a misleading `pull access denied ... repository does not exist or may require 'docker login'` rather than as a rate-limit error. The runner enumerates its `SHARD_INDEX/SHARD_TOTAL` spec slice and runs it with up to `CYPRESS_MAX_ATTEMPTS=3` retries, reaching the AIO container's DB/files via `docker exec`. A failing spec fails only that shard (`fail-fast: false`).
