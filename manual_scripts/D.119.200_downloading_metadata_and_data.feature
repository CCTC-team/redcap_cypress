  ...
  I want to see that downloading metadata and data feature is functioning as expected
   ...
   # I used pre existing project and dummy records
    ...
    # Dummy records consist of Record Id(Field name = record_id with attribute text) as 1, Name(Field name = ptname_v2_v2 with attribute text) as Tan, Name(Field name = identifier with attribute text) as 8
        ...
        Scenario: 1 D.119.200 - Download metadata & data(XML)
        ...
        Given I login to REDCap with the user "Test_Admin"
        And I create a new project named "D.119.200_downloading_metadata_and_data" by clicking on "New Project" in the menu bar, selecting "Practice / Just for fun" from the dropdown, choosing file "Project_redcap_val_Test_2024-05-31", and clicking the "Create Project" button 
